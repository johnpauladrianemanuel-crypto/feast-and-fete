            ...
            <div key={step.status} className="flex flex-col items-center gap-1" style={{ width: '25%' }}>
              <div
                className={`relative flex items-center justify-center rounded-full transition-all duration-500 ${
                  state === 'active' ? 'w-10 h-10 shadow-lg'
                    : state === 'completed' ? 'w-8 h-8' : 'w-7 h-7'
                }`}
                style={{
                  background: state === 'upcoming' ? 'var(--muted)' : step.color,
                  boxShadow: state === 'active' ? `0 0 0 4px ${step.color}33, 0 0 16px ${step.color}55` : undefined,
                }}
              >
                {state === 'active' && (
                  <span
              ...
              <span
                className={`text-xs font-medium text-center leading-tight transition-all duration-300 ${
                  state === 'active' ? 'text-foreground font-bold' : state === 'completed' ? 'text-foreground' : 'text-muted-foreground'
                }`}
                style={{ fontSize: '10px' }}
              >
                {step.etaLabel}
...
export default function OrderStatusContent() {
  ...
  const order = user ? selectedOrder : guestOrder;
  ...
  const fetchMyOrders = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data) {
        ...
        // Try to match active order or fall back to the newest order
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${order.id}` }, (payload) => {
        ...
        if (user) {
          ...
          setMyOrders((prev) => prev.map((o) => (o.id === order.id ? { ...o, ...updated } : o)));
        } else {
  ...
  const normStatus = normalizeStatus(order?.status);
  const currentStepIndex = order ? STATUS_ORDER.indexOf(normStatus) : -1;
  const progressPercent = order && normStatus !== 'cancelled'
    ? Math.max(0, Math.min(100, Math.round((Math.max(0, currentStepIndex) / (STATUS_ORDER.length - 1)) * 100)))
    : 0;

  const formattedEventDate = order?.event_date
    ? new Date(order.event_date + 'T00:00:00').toLocaleDateString('en-PH', {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 lg:px-6">
        {/* Page Header */}
        <div
        ...
        >
          ...
          <p className="text-muted-foreground text-sm">
            {user ? 'All your orders are shown below with live status updates.' : 'Enter your order number to see real-time status updates'}
          </p>
        </div>

        {/* LOGGED-IN: My Orders List + Detail */}
        {user && (
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:w-80 flex-shrink-0">
              <div className="bg-card rounded-2xl border border-border overflow-hidden" style={{ boxShadow: 'var(--shadow-3d)' }}>
                <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                  <h2 className="font-display text-sm font-bold text-foreground">My Orders</h2>
                  <span className="text-xs text-muted-foreground">{myOrders.length} total</span>
                </div>
                {myOrdersLoading && (
                ...
                {!myOrdersLoading && myOrders.length === 0 && (
                  <div className="text-center py-12 px-4">
                    ...
                    <Link href="/menu-browse-screen" className="text-xs text-primary font-medium mt-1 inline-block hover:underline">Browse our menu →</Link>
                  </div>
                )}
                {!myOrdersLoading && myOrders.length > 0 && (
                    ...
                    {myOrders.map((o) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 transition-all duration-200 hover:bg-muted/50 ${isSelected ? 'bg-primary/5 border-l-2 border-primary' : ''}`}
                        >
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <span className="font-mono text-xs font-bold text-primary">{o.order_number}</span>
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${badge.bg} ${badge.text}`}>
          ...
          <>
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. FF-20260712-ABCD"
            ...
            <div className="mb-6 bg-primary/5 border border-primary/20 rounded-2xl p-4 flex items-center gap-3">
              ...
              <p className="text-sm text-foreground">
                <Link href="/sign-up-login-screen" className="font-semibold text-primary hover:underline">Sign in</Link>
            ...
            {!guestOrder && !guestLoading && !guestError && !orderNumber && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground text-sm">Enter your order number above to track your order status in real time.</p>
                <p className="text-xs text-muted-foreground mt-2">Your order number was shown on the confirmation page (e.g. FF-20260712-ABCD)</p>
              </div>
            )}
          </>
        )}
...
function OrderDetail({
  ...
  useEffect(() => {
    if (normalizeStatus(prevStatus.current) !== normalizeStatus(order.status)) {
      ...
      const t = setTimeout(() => setStatusChanged(false), 2000);
      return () => clearTimeout(t);
    }
  }, [order.status]);
    ...
    >
      ...
      {statusChanged && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center gap-3 animate-pulse">
          ...
          <p className="text-sm font-semibold text-green-700">Order status updated to <strong className="capitalize">{normStatus}</strong>!</p>
        </div>
      )}
      ...
      <div className="bg-card rounded-2xl border border-border p-6" style={{ boxShadow: 'var(--shadow-3d)' }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-lg font-bold text-primary tracking-wider">{order.order_number}</span>
            </div>
          ...
          <div className="flex flex-col items-end gap-1">
            {normStatus === 'cancelled' ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-100 text-red-700 border border-red-200">
                <Icon name="XCircleIcon" size={14} /> Cancelled
              </span>
        ...
        {normStatus === 'cancelled' && (
          <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4">
            <p className="text-sm text-red-700 font-medium">This order has been cancelled. Please contact us if you have questions.</p>
          </div>
        )}
      </div>
            ...
            {TIMELINE_STEPS.map((step, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="pb-2">
                    <p className={`text-sm font-semibold ${state === 'active' ? 'text-primary' : 'text-foreground'}`}>
                      {step.label}
                    </p>
      ...
      <div className="bg-card rounded-2xl border border-border p-6" style={{ boxShadow: 'var(--shadow-3d)' }}>
        ...
        {/* Info Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4 border-b border-border text-xs mb-4">
          <div>
            <span className="text-muted-foreground block mb-0.5">Customer Name</span>
          ...
          <div>
            ...
            <span className="font-semibold text-foreground">{order.customer_phone || order.customer_email}</span>
          </div>
          <div>
            <span className="text-muted-foreground block mb-0.5">Delivery Method</span>
            <span className="font-semibold text-foreground capitalize">{order.delivery_method}</span>
          </div>
          <div>
            <span className="text-muted-foreground block mb-0.5">Payment Method</span>
            <span className="font-semibold text-foreground">{PAYMENT_LABELS[order.payment_method] || order.payment_method}</span>
          </div>
          {formattedEventDate && (
            <div>
              <span className="text-muted-foreground block mb-0.5">Event Date</span>
              <span className="font-semibold text-foreground">{formattedEventDate}</span>
            </div>
          )}
          {order.event_time && (
            <div>
              <span className="text-muted-foreground block mb-0.5">Event Time</span>
              <span className="font-semibold text-foreground">{order.event_time}</span>
            </div>
          )}
          {order.delivery_address && (
            <div className="sm:col-span-2">
              <span className="text-muted-foreground block mb-0.5">Delivery Address</span>
              <span className="font-semibold text-foreground">{order.delivery_address}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3 mb-4">
          {order.order_items?.map((item) => (
            <div key={item.id} className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-foreground">₱{Number(item.subtotal).toLocaleString()}</span>
            </div>
          ))}
        </div>
